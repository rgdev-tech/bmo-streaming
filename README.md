# bmo-streaming
Private personal streaming app for iOS (React Native + NestJS).
comandos importnates 
1. cd ~/rgdev-projects/bmo-streaming/apps/tv
./scripts/remote.sh
"comando para navegar desde el simulador del andrond tv desde la terminal en la computadora"
2. cd ~/rgdev-projects/bmo-streaming/apps/client 
bunx expo run:ios --device
"comando y ruta para instalar el simulador en el telefono IOS"
3. cd apps/tv && npx expo run:android --device BMO_TV
"comando para levantar el simulador del android tv desde la computadora"